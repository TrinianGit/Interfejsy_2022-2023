...
export default function CompareTrains() {
  ...
  async function trainsgetter(startTime, endTime){
      ...
      for (let time = startTime; time <= endTime; time += 3){
        await axios.get("https://css-server-trains.glitch.me/", {
            ...
            alltrains.push(response.data);
            percentage += 100.0 / slice
    ...
    await alltrains.forEach(function (trainlist, i){
                      trainlist.forEach (function (train, index){
                        let equal = false
                        trains.forEach(function(it, ind){
                          if (_.isEqual(train, it)){
                            equal = true
                          }
                        })
                        if (!equal){
  ...
  useEffect(async () => {
    if(sessionStorage.getItem("All") === '0'){
      slice = 3
      await trainsgetter((moment(sessionStorage.getItem('Time'), 'hh:mm').hours() - 3), (moment(sessionStorage.getItem('Time'), 'hh:mm').hours() + 3))
      
    }
    else {
      slice = 7
      await trainsgetter(3, 21)
              ...
              <g id="train">
                ...
                <g id="car-layers">
                  ...
                  <g id="Text">
                    ...
                    <text transform="translate(659.5 213.994)" id="Loading" className="st8 st10" fontSize="24.025">
                        Ładowanie 0%
                    </text>
                  </g>
                  <g id="ladders">
