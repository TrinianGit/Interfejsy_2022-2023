...
export default function Login() {
  ...
  useEffect(() => {
    sessionStorage.setItem("Prev", "/Login?#")
        ...
        <div className="px-5 py-5">
          <div className="Pillultrablack">
            <div className="row text-center align-items-center bigPad">
              <div className="miniform col-5">
                ...
                <form action='#' onSubmit={async () => {
                  ...
                  <div className="row px-5 pb-5">
                    <input className="form-control my-4" type="password" id='LogPass' placeholder="Hasło" onChange={() => {
                        sessionStorage.setItem("Password", document.querySelector('#LogPass').value)
                      }} required/>
                    <span class='ErrorMessage' style={{visibility: 'hidden', color: 'red'}}><h5>Niepoprawny login lub hasło</h5></span>
                  </div>
                  <div className="row px-5 Laptop pt-5">
                    <a className="fs-4 Advanced text-end" href="/ForgotPassword?#">Nie pamiętam hasła</a>
                  </div>
                  <div className="row px-5 py-4 justify-content-end">
                    <button className="button OrangeButton SmallButton" type="submit"><h6 className="py-2">Zaloguj się</h6></button>
                  </div>
                </form>
              </div>
              <div className="col-2">
              ...
              <div className="miniform col-5">
                <h2 className="my-4">Nie masz konta?</h2>
                <form action='#' onSubmit={async () => {
                    await createAccount();
                  }}>
                  <div className="row px-5">
                    <input className="form-control my-2" type="text" id="RegName" placeholder="Imię" onChange={() => {
                        sessionStorage.setItem("Name", document.querySelector('#RegName').value + " " + document.querySelector('#RegName2').value)
                      }} required/>
                  </div>
                  <div className="row px-5">
                    <input className="form-control my-2" type="text" id="RegName2" placeholder="Nazwisko" onChange={() => {
                        sessionStorage.setItem("Name", document.querySelector('#RegName').value + " " + document.querySelector('#RegName2').value)
                      }} required/>
                  </div>
                  <div className="row px-5">
                  ...
                  <div className="row px-5">
                    <input className="form-control my-2" type="password" id="RegPassword" placeholder="Hasło" onChange={() => {
                        sessionStorage.setItem("Password", document.querySelector('#RegPassword').value)
                      }} required/>
                  </div>
                  <div className="row px-5 pb-1">
                    <input className="form-control my-2" type="email" id="RegEmail" placeholder="Adres e-mail" onChange={() => {
                        sessionStorage.setItem("Email", document.querySelector('#RegEmail').value)
                      }} required/>
                    <span class='ErrorMessage2' style={{visibility: 'hidden', color: 'red'}}><h5>Istnieje już użytkownik z takim loginem</h5></span>
                  </div>
                  <div className="row px-5 pb-4 py-4 pt-5 Laptop justify-content-end">
                    <button className="button OrangeButton SmallButton" type="submit"><h6 className="py-2">Załóż konto</h6></button>
                  </div>
                </form>
              </div>
            </div>
