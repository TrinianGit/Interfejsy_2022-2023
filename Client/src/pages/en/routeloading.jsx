...
export default function MapLoading() {
  ...
  useEffect(async () => {
       await axios.get("https://css-server-stations.glitch.me/", {
                     params: {
                       B: sessionStorage.getItem("ShowCarrier"),
                       TN: sessionStorage.getItem("Show").replace(" ", "-"),
                       SP: sessionStorage.getItem("ShowSS"),
                       EP: sessionStorage.getItem("ShowES"),
                       D: sessionStorage.getItem("Date")
                     }
                     }).then(async (response) => {
                      console.log(response.data)
                      sessionStorage.setItem("Stations", JSON.stringify(response.data))
                  }).catch((error) => {
                      console.log(error)
                      window.location.href = "/en/400badrequest"
                  });
  
      window.location.href = "/en/RouteShow"
  }, []);
