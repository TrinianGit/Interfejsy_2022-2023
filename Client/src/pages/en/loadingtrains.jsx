...
export default function LoadingTrains() {
  
  
  useEffect(() => {
      let from_f = sessionStorage.getItem("From").toLowerCase().replace(/\u0142/g, "l").normalize("NFD").replace(/\p{Diacritic}/gu, "").trim().replace(" ", '-')
      let to_f = sessionStorage.getItem("To").toLowerCase().replace(/\u0142/g, "l").normalize("NFD").replace(/\p{Diacritic}/gu, "").trim().replace(" ", '-')
      let date_f = sessionStorage.getItem('Date')
      let time_f = sessionStorage.getItem('Time')
      
      axios.get("https://css-server-trains.glitch.me/", {
          params: {
              SP: from_f,
              EP: to_f,
              DT: date_f,
          ...
          sessionStorage.setItem("Trains", JSON.stringify(response.data));
