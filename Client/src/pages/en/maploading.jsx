...
export default function MapLoading() {
  ...
  useEffect(async () => {
      let brands = JSON.parse(sessionStorage.getItem("Train")).brandlogos.split('+')
      let trainnames = JSON.parse(sessionStorage.getItem("Train")).traindetails
      let startPoints = JSON.parse(sessionStorage.getItem("Train")).startStations
      let endPoints = JSON.parse(sessionStorage.getItem("Train")).endStations

      for (let i = 0; i < brands.length; i++){
        allstations.pop()
        await axios.get("https://css-server-stations.glitch.me/", {
          params: {
              D: sessionStorage.getItem("Date"),
              B: brands[i],
              SP: startPoints[i],
              EP: endPoints[i],
              TN: trainnames[i].replace(" ", "-")
          }
        }).then(async (response) => {
            console.log(response.data);
            allstations = allstations.concat(response.data)
            percentage += 30.0 / brands.length
            document.querySelector("#Loading").innerHTML = `Loading ${percentage.toFixed()}%`
        }).catch((error) => {
            console.log(error)
      ...
      console.log("READY ", allstations)
    
      for (let i = 0; i < allstations.length; i++){
        await axios.get("https://css-server-stations-list.glitch.me/latlong", {
          params: {
              N: allstations[i]
          }
        }).then(async (response) => {
            ...
            alllatlong.push(response.data)
            percentage += 70.0 / allstations.length
            document.querySelector("#Loading").innerHTML = `Loading ${percentage.toFixed()}%`
        }).catch((error) => {
            console.log(error)
            window.location.href = "/en/400badrequest"
      ...
      sessionStorage.setItem("Latlong", JSON.stringify(alllatlong));
      window.location.href = "/en/MapShow"
  }, []);
              ...
              <g id="train">
                ...
                <g id="car-layers">
                  ...
                  <g id="Text">
                    ...
                    <text transform="translate(659.5 213.994)" className="st8 st10" id="Loading" fontSize="24.025">
                        Loading 0%
                    </text>
                  </g>
                  <g id="ladders">
